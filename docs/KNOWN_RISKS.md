...
**Current mitigation**:
- `source_trust` field (`user_stated` / `assistant_inferred` / `system_generated`) distinguishes source type at the candidate level.
...
**Unresolved**: No per-memory fidelity score. `source_trust` labels the origin but does not express uncertainty or decay over successive summarization steps. No confidence degradation for derived memories. No mechanism to distinguish "user explicitly stated X" from "system inferred X from patterns" beyond the binary source_trust label. As compressors become more sophisticated, this risk grows.
...
## Risk 9: Third-Party Model and Embedding Provider Exposure

**Description**: Internal actor privacy gates and retrieval safety mechanisms operate within kiwi-mem's own retrieval layer. They do not protect content once it is sent to external LLM or embedding providers. Prompt text, conversation context, memory extraction content, and embedding inputs are transmitted to third-party APIs that sit outside the actor privacy boundary.

**Current providers in use**:
- **co.yes.vg** — receives LLM prompts, conversation content, and memory extraction payloads.
- **SiliconFlow** (`Qwen/Qwen3-Embedding-0.6B`) — receives all text sent for embedding generation.

**Why dangerous**: Content that Hermes or Telegram would be blocked from retrieving internally (due to actor privacy gates) may still be exposed to external providers when processed through the LLM or embedding pipeline. A `restricted` health memory is invisible to Hermes via the actor gate, but its embedding was generated by sending the raw text to SiliconFlow. The internal privacy boundary ends at the HTTP client calling the external API.

**Current mitigation**:
- Actor privacy gates and retrieval safety operate at the kiwi-mem retrieval layer only.
- Health data path (Hermes health-db MCP) uses a separate read-only database user.
- No provider-level privacy routing exists.

**Planned mitigation (user infrastructure)**:
- The user plans to deploy local models. Local models can serve as the processing backend for `sensitive`/`restricted` content — handling triage, summarization, redaction, and digest generation without sending data to third-party providers.
- This is infrastructure-level mitigation, not code-level enforcement. Until code-level routing is in place, the system cannot guarantee that sensitive content stays local.

**Unresolved (code-level gaps)**:
- No code-level enforcement of provider routing by `privacy_level`. The codebase does not inspect content sensitivity before choosing a provider.
- No `local-only` processing policy enforced in code — all embedding and LLM calls currently go to configured external providers regardless of content sensitivity.
- No redaction-before-provider layer.
- No outbound model-call audit log that records what content was sent to which provider, at what privacy_level.

**Future direction — Provider Boundary Policy (not implemented)**:
- A policy layer that routes model calls based on `privacy_level`, `task_type`, and `actor`:
  - `public_like` / `personal`: allowed to cloud LLM (e.g., co.yes.vg).
  - `sensitive` / `restricted`: routed to local model, or to cloud LLM with redaction.
  - `sealed`: local-only; no automatic LLM processing unless explicitly unsealed.
  - `deny`: certain task_type × privacy_level combinations may be rejected entirely.
- Local embeddings for sensitive content (eliminating third-party embedding provider exposure for `restricted`/`sensitive`).
- Outbound model-call audit log recording provider, model, content hash, and privacy_level summary per call.
...
| Provider exposure | Internal gates only | None | Provider routing, local-only, redaction, audit (all deferred) |
