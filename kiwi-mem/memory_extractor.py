...
# 检测是否为 Responses API（co.yes.vg 格式），不要拼 /chat/completions
_IS_RESPONSES_API = _RAW_BASE_URL.rstrip("/").endswith("/responses")
if _IS_RESPONSES_API:
    API_BASE_URL = _RAW_BASE_URL.rstrip("/")
elif _RAW_BASE_URL.rstrip("/").endswith("/chat/completions"):
    API_BASE_URL = _RAW_BASE_URL.rstrip("/")
else:
    API_BASE_URL = f"{_RAW_BASE_URL.rstrip('/')}/chat/completions"

# 用来提取记忆的模型（便宜的就行）
MEMORY_MODEL = os.getenv("MEMORY_MODEL", "gpt-5.4-mini")
...
async def extract_memories(messages: List[Dict[str, str]], existing_memories: List[str] = None, categories: List[str] = None, model_override: str = None, prompt_override: str = None, emotion_level: str = "normal") -> List[Dict]:
    ...
    # 判断当前 URL 是 Responses API 还是 Chat Completions
    _url_is_responses = use_api_url.rstrip("/").endswith("/responses")

    # 调用 LLM 提取记忆
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            if _url_is_responses:
                payload = {
                    "model": use_model,
                    "input": [
